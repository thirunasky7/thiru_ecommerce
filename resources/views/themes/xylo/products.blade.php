...
<section class="container mx-auto py-8 md:py-12 px-4">
   ...
   <div class="lg:hidden bg-white shadow-sm py-11 px-4 sticky top-5 z-30">
                ...
                <div id="productsGrid" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                    @foreach($products as $product)
                        ...
                        <div class="product-card-container">
                            <div class="product-card {{ !$isAvailable ? 'product-unavailable' : '' }}" data-category="{{ $categoryId }}" data-price="{{ $productPrice }}" data-rating="{{ $rating }}" data-date="{{ $product->created_at }}">
                                @if(!$isAvailable)
                                ...
                                <div class="relative overflow-hidden">
                                    ...
                                    @if($isComingSoon && !$isAvailable)
                                        <div class="absolute top-4 left-4 bg-blue-600 text-white text-xs font-semibold px-2 py-1 rounded-full">
                                            Coming Soon
                                        </div>
                                    @endif
                                </div>
                                <div class="p-5 {{ !$isAvailable ? 'opacity-80' : '' }}">
                                    <a href="{{ $isAvailable ? url('/product/'.$product->slug) : 'javascript:void(0)' }}">
                                        <h3 class="text-lg font-semibold coffee-title mb-2 {{ !$isAvailable ? 'pointer-events-none cursor-default' : '' }}">{{ $productName }}</h3>
                                    </a>
                                    ...
                                    <div class="flex justify-between items-center">
                                        <span class="text-xl font-bold text-amber-800 {{ !$isAvailable ? 'opacity-70' : '' }}">{{ $currency->symbol }}{{ number_format($productPrice, 2) }}</span>
                                        <button class="add-to-cart-btn px-4 py-2 rounded-lg text-sm font-medium {{ !$isAvailable ? 'opacity-50 cursor-not-allowed bg-gray-400 hover:bg-gray-400' : '' }}" 
    ...
    <script>
        ...
        // Unavailable product message function
        ...
        function sortProducts() {
            ...
            const productCardContainers = Array.from(productsGrid.querySelectorAll('.product-card-container'));
            ...
            productCardContainers.sort((a, b) => {
                ...
                switch(sortBy) {
                    case 'price_low':
                        return parseFloat(aCard.getAttribute('data-price')) - parseFloat(bCard.getAttribute('data-price'));
                    case 'price_high':
                        return parseFloat(bCard.getAttribute('data-price')) - parseFloat(aCard.getAttribute('data-price'));
                    case 'rating':
                        return parseFloat(bCard.getAttribute('data-rating')) - parseFloat(aCard.getAttribute('data-rating'));
                    case 'newest':
                    default:
                        return new Date(bCard.getAttribute('data-date')) - new Date(aCard.getAttribute('data-date'));
                }
            });
            
            // Re-append sorted products
            productCardContainers.forEach(container => productsGrid.appendChild(container));
        }
