        ...
        <form action="{{ route('admin.products.update', $product->id) }}" method="POST" enctype="multipart/form-data" id="productForm" novalidate>
            ...
            <div class="row mb-4">
                <div class="col-md-6">
                  ...
                  <div class="col-md-6">
                    <label class="form-label">{{ __('cms.products.is_coming_soon') }} *</label>
                    <select name="is_coming_soon" class="form-select"  required>
                        <option value="0" {{ old('is_coming_soon', $product->is_coming_soon) === 0 ? 'selected' : '' }}>{{ __('cms.products.no') }}</option>
                        <option value="1" {{ old('is_coming_soon', $product->is_coming_soon) === 1 ? 'selected' : '' }}>{{ __('cms.products.yes') }}</option>
                    </select>
                </div>
...
<div class="card p-3 mt-3 border rounded">
    <h5>{{ __('cms.products.food_menu_settings') }}</h5>
    <div class="row">
        <div class="col-md-6">
            <label class="form-label">{{ __('cms.products.is_food_menu') }} *</label>
            <select name="is_food_menu" class="form-control" id="isFoodMenu" required>
                <option value="no" {{ old('is_food_menu', $product->is_food_menu) === 'no' ? 'selected' : '' }}>
                    {{ __('cms.products.no') }}
                </option>
                <option value="yes" {{ old('is_food_menu', $product->is_food_menu) === 'yes' ? 'selected' : '' }}>
                    {{ __('cms.products.yes') }}
                </option>
            </select>
        </div>
...
<script>
    ...
    // Food menu toggle
$('#isFoodMenu').change(function() {
    const isFoodMenu = $(this).val();
    const foodMenuFields = $('#food-menu-fields');
    
    if (isFoodMenu === 'yes') {
        foodMenuFields.show();
        // Add required attributes
        foodMenuFields.find('input').attr('required', 'required');
    } else {
        foodMenuFields.hide();
        // Remove required attributes
        foodMenuFields.find('input').removeAttr('required');
    }
});

// Initialize food menu state on page load
$(document).ready(function () {
    // Check if there's old input for food menu or if product is food menu
    const isFoodMenu = '{{ old('is_food_menu', $product->is_food_menu) }}';
    if (isFoodMenu === 'yes') {
        $('#isFoodMenu').val('yes').trigger('change');
    }
});
...
$('#productForm').on('submit', function(e) {
    const productType = $('#productType').val();
    const isFoodMenu = $('#isFoodMenu').val();
    let isValid = true;
    ...
    // Validate food menu fields if applicable
    if (isFoodMenu === 'yes') {
        const foodMenuFields = [
            { selector: 'input[name="available_from_date"]', message: 'Available from date is required' },
            { selector: 'input[name="available_to_date"]', message: 'Available to date is required' },
            { selector: 'input[name="available_from_time"]', message: 'Available from time is required' },
            { selector: 'input[name="available_to_time"]', message: 'Available to time is required' }
        ];
        
        foodMenuFields.forEach(field => {
            const element = $(field.selector);
            if (!element.val().trim()) {
                element.addClass('is-invalid');
                element.after('<div class="invalid-feedback">' + field.message + '</div>');
